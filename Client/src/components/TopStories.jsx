...
function TopStories(props) {
    ...
    useEffect(() => {
        document.title = `${props.section === "home" ? '' : section} Headlines - Catch`;
        async function fetchData() {
            try {
                let url = `https://api.nytimes.com/svc/topstories/v2/world.json?api-key=${props.apiKey}`;
                let data = await fetch(url);
                let parsedData = await data.json();
                setArticles(parsedData.results);
                setSection(parsedData.section);
                document.title = `Catch- ${props.section === "home" ? '' : section} Headlines`;
            } catch (error) {
                console.log(error);
            }
        }
        ...
        <>
            {articles.length === 0 ? <Spinner /> :
                <div className="container my-3" style={{ padding: "4rem" }}>
                    <h2 id="heading" className="text-center">Recent Catch</h2>
                    {
                        <InfiniteScroll
                            pageStart={0}
